"""CLI entry point: video + city -> WGS84 position of the video.

Installed as the ``osm-localize`` console script; also runnable as
``python -m monocular_osm.cli`` or, from a checkout, ``python main.py``::

    osm-localize --video clip.mp4 --city "Ulm, Germany"   # local video file
    osm-localize --url ... --city ...                     # YouTube clip
    osm-localize                                           # default Ulm demo clip
...
from .city_inference import guess_city_from_title, slugify_submission
...
def _reconfigure_stdio() -> None:
    """Force UTF-8, line-buffered stdout/stderr (Windows cp1252 guard).

    Avoids a crash when printing emoji/umlauts to a cp1252 console and
    makes every print() land immediately in redirected log files. Called
    at the start of :func:`main`; the root ``main.py`` shim also runs it
    before importing this module, so ``python main.py`` keeps its old
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--use-tile3d", action="store_true",
                   help="enable the 3D-tile skyline channel: fetch the open "
                        ...
                        "for this use (see monocular_osm/citygml_lod2.py).")
    ...
    p.add_argument("--use-mapanything-trajectory", action="store_true",
                   help="EXPERIMENTAL (does NOT improve accuracy — see below). Use "
                        ...
                        "submap-stitching (monocular_osm/mapanything_trajectory.py): short "
    ...
    p.add_argument("--ocr-super-res", action="store_true",
                   help="Upscale + sharpen frames before OCR (monocular_osm/scene_text._upscale_sharpen) "
    ...
    p.add_argument("--coarse-from-video", action=argparse.BooleanOptionalAction,
                   ...
                   help="GPS-free coarse prior: when --osm-around is not given, derive "
                        ...
                        "video title/description (geocoded; monocular_osm/location_prior.py). Far "
    ...
    p.add_argument("--use-vpr-prior", action=argparse.BooleanOptionalAction,
                   ...
                   help="Blind coarse-location prior via Visual Place Recognition on "
                        ...
                        "monocular_osm/kartaview_vpr.py). Shape-INDEPENDENT — the fix for the "
    ...
    p.add_argument("--use-plate-anchor", action="store_true",
                   help="Blind coarse-location prior from license-plate REGISTRATION "
                        "DISTRICTS (monocular_osm/plate_anchor.py): read the EU plate region prefix "
    ...
    p.add_argument("--use-vlm-anchor", action="store_true",
                   help="VLM (Gemma 4, multimodal) district/landmark prior (monocular_osm/vlm_anchor.py): "
    ...
    p.add_argument("--use-sun-heading", action="store_true",
                   help="Recover an ABSOLUTE camera heading from the sun (monocular_osm/sun_heading.py) "
...
def main() -> None:
    ...
    for metadata, local_path in submissions:
        ...
        # title / description (monocular_osm/location_prior.py) — far tighter than the
        ...
        if frame_stride is None:
            ...
            if probe is None:
                ...
                from .pipeline import rank_cached_inputs
