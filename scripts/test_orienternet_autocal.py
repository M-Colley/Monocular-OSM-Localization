"""Validate the AUTOMATIC OrienterNet camera calibration (src.orienternet_localizer).

Calls refine_route() with focal_px=None on Ulm windows — the module sweeps
the FOV and picks it from confidence alone (no GT, no manual camera). Reports
the auto-chosen FOV and the resulting error vs the waypoint GT.
"""

from __future__ import annotations

import json

import cv2
import numpy as np
from skimage.transform import SimilarityTransform

from src.orienternet_localizer import refine_route

VIDEO = "data/ull8s4qydrk-ulm-germany-4k-drive-ulm-germany/input.mp4"
NPZ = "data/ull8s4qydrk-ulm-germany-4k-drive-ulm-germany/trajectory_v2_0-420.0_s3_fauto.npz"
GT = "ground_truth/ulm_ULl8s4qydrk.json"
STRIDE = 3
R, MPD = 6371000.0, 111320.0


def err_m(a, b):
    dlat = np.radians(a[0] - b[0]); dlon = np.radians(a[1] - b[1])
    h = np.sin(dlat / 2) ** 2 + np.cos(np.radians(b[0])) ** 2 * np.sin(dlon / 2) ** 2
    return float(2 * R * np.arcsin(np.sqrt(h)))


def main() -> None:
    xz = np.load(NPZ)["xz"]
    wps = json.load(open(GT))["waypoints"]
    cap = cv2.VideoCapture(VIDEO)
    fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
    dt = STRIDE / fps
    # Local frame for the VO->geo georeference (rotation/scale).
    lat0 = np.mean([w["lat"] for w in wps]); lon0 = np.mean([w["lon"] for w in wps])
    cl = np.cos(np.radians(lat0))
    wp_xy = np.array([[(w["lon"] - lon0) * MPD * cl, (w["lat"] - lat0) * MPD] for w in wps])
    wp_vo = np.clip([int(round(w["t_sec"] / dt)) for w in wps], 0, len(xz) - 1)
    tf = SimilarityTransform(); tf.estimate(xz[wp_vo], wp_xy)
    s = tf.scale; Rm = tf.params[:2, :2] / s

    print("Automatic OrienterNet (focal_px=None -> auto-calibrated FOV):")
    errs = []
    for k in [3, 4, 6, 9]:                       # a few central/late waypoints
        c = int(wp_vo[k])
        win = [i for i in range(c - 52, c + 53, 13) if 0 <= i < len(xz)]
        if len(win) < 4:
            continue
        geo = wp_xy[k] + s * (xz[win] - xz[c]) @ Rm.T     # metres, local frame
        prior_ll = np.c_[lat0 + geo[:, 1] / MPD, lon0 + geo[:, 0] / (MPD * cl)]
        frames = []
        for i in win:
            cap.set(cv2.CAP_PROP_POS_FRAMES, int(i * STRIDE))
            ok, bgr = cap.read()
            frames.append(bgr if ok else frames[-1])
        refined = refine_route(frames, prior_ll, None, tile_m=130.0, gravity=(0.0, -4.0))
        if refined is None:
            print(f"  waypoint t={wps[k]['t_sec']}s: OrienterNet unavailable")
            continue
        ci = len(refined) // 2
        e = err_m(refined[ci], np.array([wps[k]["lat"], wps[k]["lon"]]))
        errs.append(e)
        print(f"  waypoint t={wps[k]['t_sec']:>4}s -> error {e:.1f} m")
    cap.release()
    if errs:
        print(f"\nautomatic Ulm: median {np.median(errs):.1f} m over {len(errs)} waypoints "
              f"(no camera info supplied)")


if __name__ == "__main__":
    main()
